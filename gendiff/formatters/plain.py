...
def get_format(data, prefix=""):
    ...
    if isinstance(data['value'], list) and (data['status'] != '+'
                                            and data['status'] != '-'):
        # print(data)
        prefix += f'{data["key"]}.'
        res = ''
...
def get_string(data, prefix, data2=None):
    if data2 is not None:
        return f'Property \'{prefix}{data["key"]}\' was updated. ' \
               f'From {get_value(data["value"])} ' \
               f'to {get_value(data2["value"])}\n'

    if data["status"] == '+':
        return f'Property \'{prefix}{data["key"]}\'' \
